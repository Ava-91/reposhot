  ...
  return (
    <section className="mt-10 w-full">
      ...
      <div className="w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl shadow-black/40">
        <div
          ref={previewRef}
          className="relative aspect-video w-full overflow-hidden bg-[#0b0f16]"
        >
          <div
            aria-hidden="true"
          ...
          <div
            aria-hidden="true"
            className="absolute -right-24 -top-32 h-96 w-96 rounded-full bg-blue-500/15 blur-3xl"
          />

          <div
            aria-hidden="true"
            className="absolute -bottom-40 left-1/4 h-80 w-80 rounded-full bg-violet-500/10 blur-3xl"
          />

          <div className="relative flex h-full min-w-0 flex-col p-[5.5%]">
            <div className="flex min-w-0 items-center justify-between gap-4">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/10 text-xs font-bold text-white ring-1 ring-white/10">
                  R
                </div>

                <span className="text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-500 sm:text-xs">
                  RepoShot
                </span>
              </div>

              <div className="flex shrink-0 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[9px] font-medium text-zinc-400 sm:px-3 sm:text-[10px]">
                <svg
                  aria-hidden="true"
            ...
            <div className="mt-[6%] grid min-h-0 flex-1 grid-cols-[auto_minmax(0,1fr)] gap-4 sm:gap-6">
              <Image
                ...
                width={88}
                height={88}
                className="h-[clamp(52px,8vw,88px)] w-[clamp(52px,8vw,88px)] shrink-0 rounded-2xl border border-white/10 object-cover shadow-xl shadow-black/20"
              />

              <div className="min-w-0">
                <p className="truncate text-[10px] font-medium text-blue-300 sm:text-xs">
                  {repository.owner.login}
                </p>

                <h3 className="mt-1 truncate text-xl font-bold tracking-tight text-white sm:text-3xl">
                  {repository.name}
                </h3>

                <p className="mt-2 line-clamp-3 max-w-3xl text-xs leading-5 text-zinc-400 sm:text-sm sm:leading-6">
            ...
            <div className="mt-[4%] grid min-w-0 grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
              {repository.language && (
                <div className="min-w-0 rounded-xl border border-white/10 bg-white/[0.045] px-3 py-2.5 sm:px-4 sm:py-3">
                  <p className="text-[9px] uppercase tracking-wider text-zinc-600 sm:text-[10px]">
                    Language
                  </p>
                  <p className="mt-1 flex min-w-0 items-center gap-1.5 truncate text-xs font-semibold text-zinc-200 sm:text-sm">
                    <span
                      aria-hidden="true"
            ...
            <div className="mt-3 flex min-w-0 items-center justify-between gap-3 border-t border-white/10 pt-3">
              <div className="flex min-w-0 items-center gap-2">
                {repository.license && (
                  <span className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-2 py-1 text-[9px] font-medium text-zinc-500 sm:text-[10px]">
                    {repository.license}
                  </span>
                )}

                <span className="truncate text-[9px] text-zinc-600 sm:text-[10px]">
                  Public repository
                </span>
              </div>

              <span className="min-w-0 shrink text-right text-[9px] text-zinc-600 sm:text-[10px]">
