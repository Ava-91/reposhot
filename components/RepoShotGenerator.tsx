...
export default function RepoShotGenerator() {
  ...
  async function handleRepositorySubmit({ owner, repo }: { owner: string; repo: string }) {
    ...
    try {
      setRepository(await getRepository(owner, repo));
    } catch (error) {
      setError(error instanceof Error ? error.message : "Something went wrong while fetching the repository.");
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    ...
    if (!preset) {
      queueMicrotask(() => setError("That RepoShot preset is invalid."));
      return;
    }
  ...
  function handleRetry() {
    if (lastRepository && !loading) void handleRepositorySubmit(lastRepository);
  }
  ...
  async function handleShare() {
    ...
    const preset = encodePreset({
      owner: lastRepository.owner,
      repo: lastRepository.repo,
      theme,
      layout,
      template,
      metadata,
    ...
    <section className="flex w-full flex-col items-center">
      ...
      {!loading && !error && !repository && (
        <div className="mt-8 w-full max-w-2xl rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-6 py-8 text-center">
          ...
          <p className="mt-1 text-xs text-zinc-500">Paste a public GitHub repository URL above to generate your preview.</p>
        </div>
      )}
      {loading && (
        <div role="status" aria-live="polite" className="mt-6 flex items-center gap-3 text-sm text-zinc-400">
          <span aria-hidden="true" className="motion-safe:animate-spin h-4 w-4 rounded-full border-2 border-zinc-700 border-t-blue-400" />
          Fetching repository information...
        </div>
      )}
      {error && !loading && (
        <div role="alert" aria-live="assertive" className="mt-6 flex w-full max-w-2xl flex-col gap-3 rounded-xl border border-red-400/20 bg-red-400/5 px-4 py-3 text-sm text-red-400 sm:flex-row sm:items-center sm:justify-between">
          <p>{error}</p>
          {lastRepository && (
            <button type="button" onClick={handleRetry} className="min-h-10 shrink-0 rounded-lg border border-red-400/30 px-4 text-xs font-semibold text-red-300">
              Try again
            </button>
          )}
        </div>
          ...
          <div className="mb-4 flex flex-col gap-4 px-1">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-500">Your RepoShot preview.</p>
              </div>
