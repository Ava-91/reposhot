...
test("parses a valid GitHub repository URL", () => {
  assert.deepEqual(parseGitHubRepositoryUrl(" https://github.com/Ava-91/reposhot "), { owner: "Ava-91", repo: "reposhot" });
});

test("parses repository names with punctuation and underscores", () => {
  assert.deepEqual(parseGitHubRepositoryUrl("https://github.com/octo-org/project_name.js"), { owner: "octo-org", repo: "project_name.js" });
});

test("rejects invalid GitHub repository URLs", () => {
  const invalidUrls = ["", "reposhot", "http://github.com/Ava-91/reposhot", "https://gitlab.com/Ava-91/reposhot", "https://github.com/Ava-91/reposhot/issues", "https://github.com/Ava-91", "https://user:password@github.com/Ava-91/reposhot"];
  for (const value of invalidUrls) assert.equal(parseGitHubRepositoryUrl(value), null, value);
});
...
test("preserves long descriptions, many topics, and very large counters", () => {
  const repository = mapGitHubRepository({ ...baseRepository, name: "a".repeat(100), description: "description ".repeat(500), topics: Array.from({ length: 50 }, (_, index) => `topic-${index}`), stargazers_count: Number.MAX_SAFE_INTEGER, forks_count: 987_654_321, subscribers_count: 123_456_789, open_issues_count: 4_294_967_295 });
...
test("handles missing optional repository metadata", () => {
  const repository = mapGitHubRepository({ ...baseRepository, description: null, homepage: null, language: null, topics: [], license: null, owner: { login: "Ava-91", avatar_url: null } });
...
test("treats malformed topic payloads as an empty topic list", () => {
  const repository = mapGitHubRepository({ ...baseRepository, topics: undefined });
  assert.deepEqual(repository.topics, []);
});

test("prepares deterministic PNG export options", () => {
  assert.deepEqual(createExportOptions(1200, 675, "Ava-91", "reposhot"), { width: 1200, height: 675, filename: "reposhot-Ava-91-reposhot.png" });
});
...
test("reports GitHub rate limiting with retry information", () => {
  const headers = new Headers({ "x-ratelimit-remaining": "0", "retry-after": "30" });
  assert.equal(getGitHubRequestError(429, headers).message, "GitHub API rate limit reached. Try again in about 30 seconds.");
});

test("handles rejected GitHub requests without rate-limit headers", () => {
  assert.equal(getGitHubRequestError(500, new Headers()).message, "Unable to fetch repository information.");
});
